...
let mycookie = cookie(flavor:"dd", minutesSinceRemovalFromOven: 23)
mycookie.tempt()
...
class renting{
    ...
    init(type: String, _ isAvailable: Bool) {
        self.type = type
        self.isAvailable = isAvailable
    }
    
    enum typeOfHouse: String{
        case house, apartment, condom
    }
    func book() -> Void{
        if (isAvailable) {
            print("Booked")
...
let b1 = renting(type: "house", true)
print(b1)
