...
let yourSentence = "The \(nounArray[4]) is playing \(nounArray[5])."
let yourSillySentence = "The \(nounArray[index1]) is playing \(nounArray[index2])."
//: ### Exercise 2
...
shoutString = didYouKnowString.uppercased()
//: ### Exercise 3
...
let secondword = "jeijfjg"
print(secondword.contains("tuna"))
//: ### Exercise 6
...
//let cent = numOfPennies - (dollar * 100)
//print("$\(dollar).\(cent)")
...
let dollars = desired[s..<centStart]
let result = "$\(dollars).\(cents)"
print(result);

var prefix = desired.prefix(desired.count - 2)//chars before decimal
var suffix = desired.suffix(2)//last 2 chars
print("$\(prefix).\(suffix)")
...
for character in stringToReverse {
    reversedString = "\(character)" + reversedString//every chars goes front of empty string""
}
print(reversedString, terminator: "")//terminator: no newline, end with empty string
