...
for index in movieTitleArray{
    print(index + " with Puppets")
}
//: ### Exercise 2
...
for value in oldMountainViewRent{
    newMountainViewRent.append(value * 1.02)
}
...
for(key, value) in rapperPhoneNumbers{
    ...
    if value.contains("415") {
        let new = value.replacingOccurrences(of: "415", with: "510")
        rapperPhoneNumbers[key] = new;
    }
}
